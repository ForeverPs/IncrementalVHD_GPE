...
from model.GPE import GPE, GPET
...
def main(opt):
    ...
    old_prototype = model.highlights.weight.clone().detach_().to(device)
    ...
    for epoch in range(opt.epoch):
        ...
        for x, y in data_loader:
            ...
            logits = model(x)
            loss = criterion(logits, y) + lamda * max(distance_loss(old_prototype.detach(), model.module.highlights.weight) - gamma, 0)
            ...
            with torch.no_grad():
                temp_dist = distance_loss(old_prototype.detach(), model.module.highlights.weight)
                lamda = max(1e-2, lamda + 1e-4 * (temp_dist - gamma))
            model.train()
        ...
        if opt.local_rank == 0 and epoch % 2 == 0:
            ...
            with torch.no_grad():
                for x, y in tqdm.tqdm(val_loader):
                    ...
                    y = y.long().squeeze(0).to(device)
                    logits = model(x)
                    loss = criterion(logits, y) + lamda * max(distance_loss(old_prototype, model.module.highlights.weight) - gamma, 0)
                    val_loss += loss.item()
                    pred_cls = torch.max(logits, dim=-1)[1]
                    cls = torch.nn.Softmax(-1)(logits)[:, 1]
                    pr_auc, roc_auc, p, r, ap, acc = eval(cls, pred_cls, y)
            ...
            if val_ap >= opt.best_mAP and epoch >= opt.min_epoch:
                opt.best_mAP = val_ap
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='./tensorboard/GPE/stage_all/feat_gpe_stage2_dim_128_layer_3')
    parser.add_argument('--save_path', type=str, default='./saved_models/GPE/stage_all/feat_gpe_stage2_dim_128_layer_3')

    parser.add_argument('--checkpoint', type=str, default='./saved_models/GPE/stage1/feat_gpe_stage1_dim_128_layer_3/epoch_10_P_0.45162_R_0.64495_mAP_0.36094.pth')
    # parser.add_argument('--checkpoint', type=str, default='./saved_models/GPE/stage2/feat_gpe_stage2_dim_128_layer_3/epoch_18_P_0.42951_R_0.68083_mAP_0.35482.pth')
    # parser.add_argument('--checkpoint', type=str, default='./saved_models/GPE/stage3/feat_gpe_stage2_dim_128_layer_3/epoch_22_P_0.34296_R_0.78910_mAP_0.30865.pth')
...
# python -m torch.distributed.launch --nproc_per_node=4 incremental_train.py --n_gpus=4